defmodule MarsRoverTest do
  ...
  test "rotations" do
    ...
    assert MarsRover.process(position_facing_north, :left) == {0,0,:west}
    assert MarsRover.process(position_facing_north, :right) == {0,0,:east}

    position_facing_south = {0,0,:south}
    assert MarsRover.process(position_facing_south, :left) == {0,0,:east}
    assert MarsRover.process(position_facing_south, :right) == {0,0,:west}

    position_facing_west = {0,0,:west}
    assert MarsRover.process(position_facing_west, :left) == {0,0,:south}
    assert MarsRover.process(position_facing_west, :right) == {0,0,:north}

    position_facing_east = {0,0,:east}
    assert MarsRover.process(position_facing_east, :left) == {0,0,:north}
    assert MarsRover.process(position_facing_east, :right) == {0,0,:south}
  end
end
