defmodule MarsRover.DeploymentsTest do
  ...
  test "move north from 0,0,N" do
    ...
    {:ok, final_position} = Deployments.run_commands(position_heading_north, @plateau, [:move])
  ...
  test "rotate 180 degress" do
    ...
    {:ok, final_position} = Deployments.run_commands(position_heading_north, @plateau, commands)
  ...
  test "rotate 270 degress" do
    ...
    {:ok, final_position} = Deployments.run_commands(position_heading_south, @plateau, commands)
  ...
  test "move from 0,0,N to 3,2,S" do
    ...
    {:ok, final_position} = Deployments.run_commands({0, 0, :north}, @plateau, commands)
  ...
  test "move from 3,2,S to 0,0,N" do
    ...
    {:ok, final_position} = Deployments.run_commands({3, 2, :south}, @plateau, commands)
  ...
  test "move from upper-right 4,6,W in a square and come back" do
    ...
    {:ok, final_position} = Deployments.run_commands(upper_right, @plateau, commands)
    assert final_position == upper_right
  end

  test "start from outside the plateau -9,-9,N" do
    commands = []
    result = Deployments.run_commands({-9, -9, :north}, @plateau, commands)
  ...
  test "move from 3,2,N to -2,3,W offgrid" do
    commands = ~W/move left move move move move move/a
    result = Deployments.run_commands({3, 2, :north}, @plateau, commands)
    assert result == {:error, :off_plateau}
  end

  test "move from 3,5,W to 6,5,E offgrid and back" do
    commands = ~W/left left move move move left left move move move/a
    result = Deployments.run_commands({3, 5, :west}, @plateau, commands)
    assert result == {:error, :off_plateau}
  end
  ...
  test "processing several deployments" do
    ...
    assert Deployments.process_deployments(plateau, deployments) == [{:ok, {3, 3, :north}}]
  end
end
