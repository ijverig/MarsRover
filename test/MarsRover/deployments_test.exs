defmodule MarsRover.DeploymentsTest do
  ...
  test "move north from 0,0,N" do
    ...
    {:ok, final_position} = Deployments.deploy(position_heading_north, @plateau, [:move])
  ...
  test "rotate 180 degress" do
    ...
    {:ok, final_position} = Deployments.deploy(position_heading_north, @plateau, commands)
  ...
  test "rotate 270 degress" do
    ...
    {:ok, final_position} = Deployments.deploy(position_heading_south, @plateau, commands)
  ...
  test "move from 0,0,N to 3,2,S" do
    ...
    {:ok, final_position} = Deployments.deploy({0, 0, :north}, @plateau, commands)
  ...
  test "move from 3,2,S to 0,0,N" do
    ...
    {:ok, final_position} = Deployments.deploy({3, 2, :south}, @plateau, commands)
  ...
  test "move from upper-right 4,6,W in a square and come back" do
    ...
    {:ok, final_position} = Deployments.deploy(upper_right, @plateau, commands)
    assert final_position == upper_right
  end

  test "start from outside the plateau -9,-9,N" do
    commands = []
    result = Deployments.deploy({-9, -9, :north}, @plateau, commands)
  ...
  test "processing several deployments" do
    ...
    assert Deployments.deploy_several(plateau, deployments) == [{:ok, {3, 3, :north}}]
  end
end
