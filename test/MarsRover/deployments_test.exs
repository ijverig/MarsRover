defmodule MarsRover.DeploymentsTest do
  ...
  test "move north from 0,0,N" do
    ...
    {:ok, final_position} = D.deploy(position_heading_north, [:move], @plateau, [])
  ...
  test "rotate 180 degress" do
    ...
    {:ok, final_position} = D.deploy(position_heading_north, commands, @plateau, [])
  ...
  test "rotate 270 degress" do
    ...
    {:ok, final_position} = D.deploy(position_heading_south, commands, @plateau, [])
  ...
  test "move from 0,0,N to 3,2,S" do
    ...
    {:ok, final_position} = D.deploy({0, 0, :north}, commands, @plateau, [])
  ...
  test "move from 3,2,S to 0,0,N" do
    ...
    {:ok, final_position} = D.deploy({3, 2, :south}, commands, @plateau, [])
  ...
  test "move from upper-right 4,6,W in a square and come back" do
    ...
    {:ok, final_position} = D.deploy(upper_right, commands, @plateau, [])
    assert final_position == upper_right
  end

  test "start from outside the plateau -9,-9,N" do
    commands = []
    result = D.deploy({-9, -9, :north}, commands, @plateau, [])
  ...
  test "processing several deployments" do
    ...
    assert D.deploy_several(deployments, plateau) == [{:ok, {3, 3, :north}}]
  end
end
